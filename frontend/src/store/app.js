export const app = {
	...
	getters: {
		isDialogOpen(state) {
			if(state.pageDialog.showDialog || state.rightDrawer.showDrawer){
				return true;
			}
			return false
		},
	...
	mutations: {
		...
		setRightDrawer(state, payload) {
			if(payload.showDrawer){
				...
				state.rightDrawer.pageData = payload.pageData;
			}
		...
		setFullScreenLoadingMsg(state, value) {
			state.fullScreenLoadingMsg = value;
		},
	},
	actions: {
