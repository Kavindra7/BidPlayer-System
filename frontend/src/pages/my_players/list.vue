                ...
                <div class="container-fluid">
                    <div class="grid justify-content-between">
                        ...
                        <div class="col-12 md:col-3 lg:col-2 " >
                            <div class="">
                                <template v-if="$auth.canView('/my_players/add')">
                                    <Button label="Add New Player" icon="pi pi-plus"  @click="app.openPageDialog({ page: 'my_players/add', url: `/my_players/add` , closeBtn: true })"  class="w-full bg-primary "  />
                                </template>
                            </div>
                        </div>
                            ...
                            <div class="">
                                <div >
                                    <template v-if="showBreadcrumbs && $route.query.tag">
                                        <Breadcrumb :home="{icon: 'pi pi-home', to: '/my_players'}" :model="pageBreadCrumb" />
                                    </template>
                                                ...
                                                <div class="page-records" >
                                                    <DataTable :lazy="true" :loading="loading"    v-model:selection="selectedItems"
                                                        ...
                                                        <Column  field="id" header="Id" >
                                                            <template #body="{data}">
                                                                <router-link :to="`/my_players/view/${data.id}`">
                                                                    {{ data.id }}
                                                                </router-link>
                                                            </template>
                                                        </Column>
                                                        <Column  field="player_name" header="Player Name" >
                                                            <template #body="{data}">
                                                                <Button class="p-button-text" icon="pi pi-eye" label="View Player" v-if="data.player_name" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.player_name}` , closeBtn: true })" />
                                                            </template>
                                                        </Column>
                                                        ...
                                                        <Column  field="bidder_id" header="Bidder" >
                                                            <template #body="{data}">
                                                                <Button class="p-button-text" icon="pi pi-eye" :label="data.users_username" v-if="data.bidder_id" @click="app.openPageDialog({ page: 'users/view', url: `/users/view/${data.bidder_id}` , closeBtn: true })" />
                                                            </template>
                                                        </Column>
                                                        ...
                                                        <div v-show="pageReady">
                                                            <div class="flex justify-content-between">
                                                                <div class="flex justify-content-center flex-grow-0">
                                                                    <template v-if="$auth.canView('my_players/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'my_playerslist',
		},
		apiPath : {
			type : String,
			default : 'my_players/index',
		},
		paginate: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/my_players/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('my_players/view')
		},
		{
			label: "Edit",
