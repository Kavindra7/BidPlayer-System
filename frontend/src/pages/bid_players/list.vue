            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between">
                        ...
                        <div v-if="$auth.userRole != 'Team Owner'" class="col-12 md:col-3 lg:col-2 " >
                            <div class="">
                                <template v-if="$auth.canView('/bid_players/add')">
                                    <Button label="Bid New Player" icon="pi pi-plus"  @click="app.openPageDialog({ page: 'bid_players/add', url: `/bid_players/add` , closeBtn: true })"  class="w-full bg-primary "  />
                                </template>
                            </div>
                        </div>
                            ...
                            <div class="">
                                <div >
                                    <template v-if="showBreadcrumbs && $route.query.tag">
                                        <Breadcrumb :home="{icon: 'pi pi-home', to: '/bid_players'}" :model="pageBreadCrumb" />
                                    </template>
                                                ...
                                                <div class="page-records" >
                                                    <DataTable :lazy="true" :loading="loading"    v-model:selection="selectedItems"
                                                        ...
                                                        <Column  field="id" header="Id" >
                                                            <template #body="{data}">
                                                                <router-link :to="`/bid_players/view/${data.id}`">
                                                                    {{ data.id }}
                                                                </router-link>
                                                            </template>
                                                        </Column>
                                                        <Column  field="owner_id" header="Owner" >
                                                            <template #body="{data}">
                                                                <Button class="p-button-text" icon="pi pi-eye" label="Users" v-if="data.owner_id" @click="app.navigateTo(`/users/view/${data.owner_id}`)" />
                                                            </template>
                                                        </Column>
                                                        <Column  field="player_id" header="Player" >
                                                            <template #body="{data}">
                                                                <Button class="p-button-text" icon="pi pi-eye" :label="data.users_full_name" v-if="data.player_id" @click="app.navigateTo(`/users/view/${data.player_id}`)" />
                                                            </template>
                                                        </Column>
                                                        <Column  field="thophy_id" header="Thophy" >
                                                            <template #body="{data}">
                                                                <Button class="p-button-text" icon="pi pi-eye" :label="data.trophies_trophy_name" v-if="data.thophy_id" @click="app.navigateTo(`/trophies/view/${data.thophy_id}`)" />
                                                            </template>
                                                        </Column>
                                                        ...
                                                        <div v-show="pageReady">
                                                            <div class="flex justify-content-between">
                                                                <div class="flex justify-content-center flex-grow-0">
                                                                    <template v-if="$auth.canView('bid_players/delete')">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName : {
			type : String,
			default : 'bid_playerslist',
		},
		apiPath : {
			type : String,
			default : 'bid_players/index',
		},
		paginate: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/bid_players/view/${data.id}`,
			icon: "pi pi-eye",
			visible: auth.canView('bid_players/view')
		},
		{
			label: "Edit",
