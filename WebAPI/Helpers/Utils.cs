...
public static class Utils
    {
         ...
         * @return  string
         */
        public static string RandomNum(int limit = 12, string context = "1234567890")
        {
            return new string(Enumerable.Repeat(context, limit)
              .Select(s => s[random.Next(s.Length)]).ToArray());
        }
        ...
        public static List<string> RandomColors(int limit, int alpha = 1)
        {
            var colors = new List<string>();
            for(int i=0; i<limit; i++){
                colors.Add(RandomColor(alpha));
            }
            return colors;
        }
    }
